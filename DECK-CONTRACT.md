...
- `urokimeditacii` — **опциональный** объект слайда «Уроки медитации». Печётся `build/urokimeditacii.py` **только** на неделях, где «Коллективная медитация» (Airtable, `Тип`==УМ) той же ISO-недели, что дата ДЧ, приходится на **воскресенье**. Иначе ключа нет → слайда нет. Календарь = все события месяца **только `Тип`==УМ** (АМ отбрасываются целиком), по одной ячейке на день; `kind` из имени по словарю `build/urokimeditacii.json` (`km`/`asana`/`fk`/`charity`, фолбэк `other`). Форма:
  ```json
  { "month_label": "август 2026", "month": 8, "year": 2026,
    "greeting": "<конст.>", "schedule_note": "<конст.>",
    "wisdom": "<провалидированная строка>", "channel": "t.me/…",
    "highlight_day": 30,
    "meeting": { "date_label": "вс 30 августа",
                 "items": [ {"time":"11:00","name":"Коллективная медитация","km":true},
                            {"time":"12:00","name":"Чаепитие"} ] },
    "calendar": [ { "day": 1, "kind": "km", "label": "КМ · Киноклуб" } ] }
  ```
  - `greeting` / `schedule_note` / `channel` — **константы проекта** из `build/urokimeditacii.json` (НЕ из поста).
  - `wisdom` — **единственная точка LLM этой фичи** (Мак-only, `build/wisdom.py`): валидатор → дедуп по `build/wisdom_history.json` → фолбэк-пул. Сырой ответ LLM в дек не попадает.
  - `meeting` — расписание встречи из **отдельного поста канала Уроков** (`t.me/urokimeditacii_rnd`, «План/Расписание мероприятия»), покрывающего дату КМ той же ISO-недели (`build/fetch_telegram.py:fetch_um_meeting`). НЕ афиша недели, НЕ ДЧ-пост. Строки «время · точка-kind · название» в **строгом хронопорядке по времени**; КМ выделяется акцентом (сиреневая точка + weight 500), НЕ позицией. Тонкая дата-подпись (`date_label`) над строками. Пост не найден / формат иной / нет времён → `meeting=null` → карточку расписания НЕ рисуем, слайд цел. (Правило «КМ первым» остаётся только для ячеек календаря — там времён нет.)
  - `highlight_day` — день воскресной КМ; остаётся в данных, но **рендер v11 его НЕ выделяет** (обычная km-ячейка). Ячейка календаря: акцент = самый значимый kind дня (km>asana>fk>charity>other), подпись из `Заголовок` (иначе имя); одноднёвные события через ` · `, максимум 2, дальше `+N`, КМ — первым.
...
- `overrides.hide_urokimeditacii` — `bool`, дефолт `false`. `true` → слайд «Уроки медитации» НЕ показывается, даже если manifest его испёк. Тумблер в форме виден **только** когда `manifest.urokimeditacii` присутствует. Хранится в JSONB `overrides` (без миграции БД — `selection.overrides || EXCLUDED.overrides`), переживает пересборку manifest.
...
urokimeditacii = manifest.urokimeditacii ?? null      // печётся только на неделях с воскресной КМ (Тип УМ)
show_urokimeditacii = urokimeditacii && !overrides.hide_urokimeditacii  // добавляется ПОСЛЕДНИМ в дек
```
...
| urokimeditacii | `slides/urokimeditacii.js` | manifest.urokimeditacii (календарь, wisdom, QR) | ключа нет / тумблер скрыл → слайда нет |

Слайд `urokimeditacii` — **опциональный, в самом конце** дека (после final), вне фиксированного порядка `welcome→…→final`. Светлая палитра, все стили под `.um-slide` (в тёмный дек не течёт).
