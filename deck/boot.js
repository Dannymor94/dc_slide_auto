...
// Parse a bg-music URL → { id, list }. Keeps radio mixes (RD…) and real playlists
// (PL/UU/OL…) so the queue advances; drops only private lists (LL/WL) that can't embed.
function parseBgSource(url) {
  try {
    ...
    const list = raw && !/^(LL|WL)/.test(raw) ? raw : null;
...
function setupBgMusic(url) {
  ...
  loadYTApi(() => {
    ...
    // origin MUST be set so the IFrame API can postMessage commands (nextVideo/pause) into
    // the player. Letting YT.Player CREATE the iframe (mount div) makes the load handshake
    // correct; wrapping a pre-made <iframe src> races it → "postMessage target origin does
    // not match" and every command silently no-ops (the track never advances).
    const playerVars = { controls: 1, playsinline: 1, rel: 0, origin: location.origin };
    let videoId;
    if (src.list && /^(PL|UU|OL)/.test(src.list)) {
      // Real playlist: queue is the list itself, no seed video (a seed pins to one track).
      playerVars.listType = 'playlist';
      playerVars.list = src.list;
      playerVars.loop = 1;
    } else if (src.list) {
      // Radio mix (RD…): seed video + list build the radio; no listType (RD isn't a playlist).
      videoId = src.id || undefined;
      playerVars.list = src.list;
    } else {
      videoId = src.id || undefined;   // single video
    }
      ...
      bgPlayer = new YT.Player(mount, {
        ...
        events: {
          onStateChange: e => {
            ...
            // auto-advance (gestureless play → pause). Force the next track ourselves —
            // allowed because the first native click already "activated" the player.
